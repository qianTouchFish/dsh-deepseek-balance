...
const STATE_FILENAME = "dsh-deepseek-balance.json";

/**
 * Describe a fetch transport failure, including undici's `cause.code`
 * (ENOTFOUND / ECONNREFUSED / SELF_SIGNED_CERT_IN_CHAIN / ...), so the card
 * shows an actionable reason instead of a bare "fetch failed".
 */
...
export async function collectStatus(ctx, keyRef) {
	...
	let response;
	try {
		response = await fetch(`${API_BASE}/user/balance`, {
			headers: { authorization: `Bearer ${apiKey}`, accept: "application/json" },
			signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS)
		});
	} catch (error) {
		throw new Error(`DeepSeek API 连接失败: ${transportDetail(error)}`);
	}
	if (!response.ok) {
