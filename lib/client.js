window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ---- theme-aware palettes (light / dark) ----
		function isLightSurface() {
			try {
				// the harness signals dark via body[data-ds-dark-theme]; the
				// --dsw-alias-* variables are defined on <body>, not <html>.
				if (document.body.hasAttribute("data-ds-dark-theme")) return false;
				const raw = getComputedStyle(document.body).getPropertyValue("--dsw-alias-bg-layer-1").trim();
				let m = raw.match(/^#([0-9a-fA-F]{6})/);
				if (m) {
					const n = parseInt(m[1], 16);
					return (((n >> 16) & 255) * 299 + ((n >> 8) & 255) * 587 + (n & 255) * 114) / 1000 > 128;
				}
				m = raw.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);
				if (m) return (Number(m[1]) * 299 + Number(m[2]) * 587 + Number(m[3]) * 114) / 1000 > 128;
				return true; // base body variables are the light set
			} catch { }
			return true;
		}
		function palette() {
			const font = '"Inter","PingFang SC","Microsoft YaHei",system-ui,-apple-system,BlinkMacSystemFont,sans-serif';
			return isLightSurface() ? {
				...
				menuShadow: "0 8px 20px rgba(0,0,0,0.15)",
				font
			} : {
				bg: "#0b0c0f",
				card: "#18191d",
				...
				menuShadow: "0 8px 20px rgba(0,0,0,0.4)",
				font
			};
		}
		...
		// ---- sidebar strip (always visible, above Settings; 4 metrics in a horizontal row) ----
		function StatusStrip({ t, state, period, onClick, stripRef }) {
			...
			const item = (label, value, strong) => react_jsx_runtime.jsxs("span", {
				style: { display: "inline-flex", alignItems: "baseline", gap: 2, whiteSpace: "nowrap", fontSize: strong ? 10 : 9, fontWeight: strong ? 700 : 400, color: strong ? color : C.muted, lineHeight: "14px" },
				children: [react_jsx_runtime.jsx("span", { children: label }), react_jsx_runtime.jsx("span", { style: { color: strong ? color : C.fg, fontWeight: strong ? 700 : 600 }, children: value })]
			});
		...
		function PeakBadge({ t }) {
			...
			const color = peak ? C.warn : C.ok;
			return react_jsx_runtime.jsx("span", {
				title: t("peakWindow"),
				style: { display: "inline-flex", alignItems: "center", gap: 3, borderRadius: 999, padding: "1px 6px", fontSize: 9, fontWeight: 700, lineHeight: "14px", color, background: peak ? "rgba(245,158,11,0.14)" : "rgba(34,197,94,0.14)", border: `1px solid ${peak ? "rgba(245,158,11,0.45)" : "rgba(34,197,94,0.45)"}`, whiteSpace: "nowrap", flex: "none" },
				children: [
					react_jsx_runtime.jsx("span", { "aria-hidden": "true", style: { width: 5, height: 5, borderRadius: "50%", background: color, flex: "none" } }),
						...
						react_jsx_runtime.jsxs("div", {
							...
							children: [
								...
								menuOpen ? react_jsx_runtime.jsx("div", {
									style: { position: "absolute", top: "calc(100% - 4px)", left: 12, zIndex: 10001, background: C.card2, border: `1px solid ${C.border}`, borderRadius: 10, boxShadow: C.menuShadow, padding: 4, display: "flex", flexDirection: "column", minWidth: 108 },
								...
								keyMenu ? react_jsx_runtime.jsx("div", {
									style: { position: "absolute", top: "calc(100% - 4px)", right: 12, zIndex: 10001, background: C.card2, border: `1px solid ${C.border}`, borderRadius: 10, boxShadow: C.menuShadow, padding: 4, display: "flex", flexDirection: "column", minWidth: 170, maxHeight: 200, overflowY: "auto" },
							...
							react_jsx_runtime.jsxs("header", {
								...
								children: [
									...
									modelMenu ? react_jsx_runtime.jsx("div", {
										style: { position: "absolute", top: "calc(100% + 2px)", left: 16, zIndex: 10001, background: C.card2, border: `1px solid ${C.border}`, borderRadius: 10, boxShadow: C.menuShadow, padding: 4, display: "flex", flexDirection: "column", minWidth: 170, maxHeight: 240, overflowY: "auto" },
