window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function StatusStrip({ wide, t, state, period, onClick }) {
			...
			if (!wide) {
				// collapsed rail: compact 4-metric mini stack
				return react_jsx_runtime.jsx("button", {
					...
					children: [
						...
						react_jsx_runtime.jsx("span", { style: { fontSize: 8, color: C.muted, lineHeight: "10px", whiteSpace: "nowrap" }, children: `${t("spent")} ${fmtMoney(periodData?.cost ?? null)}` }),
						react_jsx_runtime.jsx("span", { style: { fontSize: 8, color: C.muted, lineHeight: "10px", whiteSpace: "nowrap" }, children: `${t("apiRequests")} ${fmtInt(periodData?.requests ?? null)}` }),
						react_jsx_runtime.jsx("span", { style: { fontSize: 8, color: C.muted, lineHeight: "10px", whiteSpace: "nowrap" }, children: `${t("tokens")} ${fmtTokensCompact(periodData?.tokens ?? null)}` })
					]
				});
			}
			return react_jsx_runtime.jsx("button", {
		...
		// ---- platform token input row (auto fetch / manual save) ----
		function TokenRow({ t, refresh }) {
			...
			const run = async (action, token) => {
				...
				try {
					...
					if (result.ok && result.found) {
						setDraft("");
						setMsg(`${t("tokenFetched")} ${result.masked}`);
						refresh();
					} else if (result.ok) {
						setDraft("");
						setMsg(`${t("tokenSaved")} ${result.masked}`);
						refresh();
					} else {
						setMsg(result.error ?? t("tokenFailed"));
					}
				} catch (error) {
					setMsg(String(error));
				} finally {
			...
			return react_jsx_runtime.jsxs("div", {
				style: { display: "flex", gap: 5, alignItems: "center", padding: "6px 12px", borderTop: `1px solid ${C.border}`, background: C.bg },
				children: [
					...
					react_jsx_runtime.jsx("button", {
						...
						onClick: () => run("auto"),
						disabled: busy,
						style: { border: "none", background: C.mutedBg, color: C.fg, borderRadius: 6, padding: "3px 6px", cursor: "pointer", fontFamily: C.font, fontSize: 9, flex: "none" },
					...
					react_jsx_runtime.jsx("button", {
						...
						style: { border: "none", background: C.primary, color: "#fff", borderRadius: 6, padding: "3px 8px", cursor: "pointer", fontFamily: C.font, fontSize: 9, flex: "none" },
						children: t("tokenSave")
					}),
					msg ? react_jsx_runtime.jsx("span", { style: { color: C.muted, fontSize: 8, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 120 }, title: msg, children: msg }) : null
				]
			});
		}

		// ---- the reference-style card ----
		function StatusCard({ wide, t, state, refresh, onClose, period, onPeriodChange, keyRef, onSelectKey }) {
			...
			const activeModelId = currentModel ?? (selectorOptions.length > 0 ? selectorOptions[0].model : null);
				...
				children: [
					...
					react_jsx_runtime.jsxs("div", {
						...
						children: [
									...
									modelMenu ? react_jsx_runtime.jsx("div", {
										...
										children: selectorOptions.length === 0
											...
											: selectorOptions.map((opt) => {
												...
												return react_jsx_runtime.jsx("button", {
													type: "button",
													onClick: () => { setCurrentModel(opt.model); setModelMenu(false); },
							...
							body,
							react_jsx_runtime.jsx(TokenRow, { t, refresh }),
							react_jsx_runtime.jsxs("div", {
		...
		function DeepSeekStatus({ wide, t }) {
			const [keyRef, setKeyRef] = react.useState(null);
