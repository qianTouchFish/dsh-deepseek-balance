window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function isLightSurface() {
			try {
				const raw = getComputedStyle(document.documentElement).getPropertyValue("--dsw-alias-bg-layer-1").trim();
		...
		function useThemeTick() {
			...
			react.useEffect(() => {
				...
				try {
					...
					observer.observe(document.documentElement, { attributes: true, attributeFilter: ["class", "data-theme", "data-color-scheme", "style"], childList: false, subtree: false });
				} catch { }
				return () => {
