window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ---- module-level persisted selections (survive card open/close and remounts) ----
		const persisted = { period: "days30", keyRef: null, model: null };
		...
		function DeepSeekStatus({ wide, t }) {
			...
			const setPeriod = (value) => { persisted.period = value; setPeriodState(value); };
			const setKeyRef = (value) => { persisted.keyRef = value; setKeyRefState(value); };
			const setCurrentModel = (value) => { persisted.model = value; setCurrentModelState(value); };
