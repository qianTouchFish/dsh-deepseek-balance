window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function StatusCard({ t, state, refresh, onClose }) {
			...
			const balance = data?.balance ?? null;
			const usage = data?.usage ?? null;
			const modelGroups = data?.models ?? [];
			const modelChoices = modelGroups.flatMap((group) => group.models.map((m) => ({ provider: group.provider, id: m.id, name: m.name ?? m.id })));
			const activeModel = currentModel ?? (modelChoices.length > 0 ? modelChoices[0] : null);
			const low = balance !== null && balance.infos.some((info) => info.total < LOW_BALANCE_THRESHOLD);
						...
						react_jsx_runtime.jsxs("div", {
							...
							children: [
								react_jsx_runtime.jsxs("button", {
									type: "button",
									onClick: () => { setMenuOpen(!menuOpen); setModelMenu(false); },
			...
			return react_jsx_runtime.jsxs(react.Fragment, {
				children: [
					menuOpen || modelMenu ? react_jsx_runtime.jsx("div", {
						onClick: () => { setMenuOpen(false); setModelMenu(false); },
						...
						children: [
							react_jsx_runtime.jsx("header", {
								...
								children: [
									...
									react_jsx_runtime.jsxs("button", {
										type: "button",
										onClick: () => { setModelMenu(!modelMenu); setMenuOpen(false); },
										...
										children: [
											react_jsx_runtime.jsx("span", { style: { whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }, children: activeModel ? activeModel.id : t("selectModel") }),
											react_jsx_runtime.jsx("span", { style: { color: C.muted, fontSize: 9 }, children: "▾" })
										]
									}),
									react_jsx_runtime.jsxs("span", {
										style: { display: "inline-flex", alignItems: "baseline", gap: 4, background: C.mutedBg, border: "none", borderRadius: 999, padding: "2px 8px", color: C.fg, fontSize: 9, minWidth: 0 },
										children: [
											react_jsx_runtime.jsx("span", { style: { color: C.muted }, children: t("keys") }),
											react_jsx_runtime.jsx("span", { style: { whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontWeight: 600 }, children: fmtKey(data?.key?.masked ?? null) })
										]
									}),
									...
									modelMenu ? react_jsx_runtime.jsx("div", {
										style: { position: "absolute", top: "calc(100% + 2px)", left: 16, zIndex: 10001, background: C.card2, border: `1px solid ${C.border}`, borderRadius: 10, boxShadow: "0 8px 20px rgba(0,0,0,0.4)", padding: 4, display: "flex", flexDirection: "column", minWidth: 150, maxHeight: 240, overflowY: "auto" },
										children: modelGroups.length === 0
											...
											: modelGroups.flatMap((group) => [
												react_jsx_runtime.jsx("div", { style: { padding: "4px 8px", color: C.muted, fontSize: 9, fontWeight: 600 }, children: group.name ?? group.provider }, `h-${group.provider}`),
												group.models.map((m) => {
													const selected = activeModel !== null && activeModel.provider === group.provider && activeModel.id === m.id;
													return react_jsx_runtime.jsx("button", {
														type: "button",
														onClick: () => { setCurrentModel({ provider: group.provider, id: m.id, name: m.name ?? m.id }); setModelMenu(false); },
