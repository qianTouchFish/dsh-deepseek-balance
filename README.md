...
- 侧边栏"设置"上方**常显余额条**(无需点击即可看到):`余额 ¥X.XX · 今日 ≈¥X.XX` 与 `累计 ≈¥X.XX · tokens · 请求次数`,每分钟自动刷新;
- 点击余额条展开**详情卡片**:余额明细(总额 / 赠送 / 充值)、可用状态、**今日 / 本月 / 累计消费**、**今日 / 本月 Tokens**、**今日 / 本月请求次数**、密钥掩码与来源、请求耗时、手动刷新;
- **低余额告警**(默认阈值 ¥10,余额条与卡片变黄);
- 数据来源透明:余额为官方实时接口;消费为余额差值估算(持久化、充值自动校正、显示"≈");Tokens/请求次数为**官方平台用量接口**(与平台控制台一致);
- 深 / 浅色主题自适应(`--dsw-*` 设计变量);
